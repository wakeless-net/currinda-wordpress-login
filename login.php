...
define("CURRINDA_VERSION", "0.2");

require plugin_dir_path(__FILE__) . 'vendor/autoload.php';
require plugin_dir_path(__FILE__) . '/CurrindaProvider.php';
require dirname(__FILE__) . '/login_afo_widget.php';
...
?>
  ...
  { ?>
    <p>
      <font color="#FF0000"><strong>Note*</strong></font>
      <br />
...
<?php
  ...
  function login_validate()
  {
    if (isset($_GET['option']) and $_GET['option'] == "currinda_user_login") {
      ...
      if (!isset($_GET['code'])) {
        ...
        $this->error = new WP_Error("404", "Token Expired. Please refresh your browser.");
        return $this->error;
      } else {
        ...
        try {
          ...
          $this->setup_user_or_login($resourceOwner->toArray());
        ...
        //   $details = $provider->getResourceOwner($token);
  ...
  {
    $_SESSION["CurrindaLogin"]["AccessToken"] = $token->getToken();
    $_SESSION["CurrindaLogin"]["RefreshToken"] = $token->getRefreshToken();
  }
  ...
  {
    if (
      !$this->has_expiry_date_past($details->Membership->ExpiryDate) &&
  ...
  function is_a_corp_member($details)
  {
    ...
    foreach ($details->CorporateMemberships as $corp_member) {
      if (
        !$this->has_expiry_date_past($corp_member->ExpiryDate) &&
  ...
  {
    if (!isset($details->Membership->Parent)) {
      return false;
    }
    if (
      !$this->has_expiry_date_past($details->Membership->Parent->ExpiryDate) &&
  ...
  function check_valid_record($details)
  {
    ...
    if ($this->is_a_standard_member($details)) {
      return true;
    }
    if ($this->is_a_corp_member($details)) {
      return true;
    }
    if ($this->is_a_sub_member($details)) {
      return true;
    }
  ...
  function provider()
  {
    ...
    $returnOauth =  new GenericProvider(array(
      'clientId'  =>  $this->client_id,
      'clientSecret'  =>  $this->client_secret,
      "scopes" => ["user"],
      'redirectUri' => $return_url,
      'urlAuthorize' => $url . "api/$version/organisation/$this->scope/authorize",
      "urlAccessToken" => $url . "api/$version/organisation/$this->scope/token",
      "urlResourceOwnerDetails" => $details_url
    ));
    return $returnOauth;
  }
